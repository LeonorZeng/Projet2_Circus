...
/* Tri des scores: score d�croissant, puis nom croissant */
typedef struct {
...
static int cmp_scoreline(const void* a, const void* b) {
    ...
    if (A->score != B->score) return (B->score - A->score); /* d�croissant */
    return strcmp(A->nom, B->nom);                           /* alphab�tique */
}
...
/* =========================
   Ligne d'ordres autoris�s
   ========================= */
   /* Doit afficher la ligne en se limitant aux ordres autoris�s :contentReference[oaicite:9]{index=9} */
static void print_ligne_ordres(const Lecture* lec) {
    int first = 1;

    /* ordre d'affichage demand� dans le sujet */
    if (lec->allow_KI) {
        if (!first) printf(" | ");
        printf("KI (B -> R)");
        first = 0;
...
/* Affiche 2 jeux c�te � c�te selon le format de l��nonc� :contentReference[oaicite:11]{index=11} */
...
int main(int argc, const char* argv[]) {
    ...
    /* v�rif: animaux >= 2, ordres >= 3 (sinon stop imm�diat) :contentReference[oaicite:14]{index=14} */
    ...
    /* 3) Afficher la ligne des ordres autoris�s :contentReference[oaicite:15]{index=15} */
    print_ligne_ordres(&lec);

    /* 4) G�n�rer toutes les cartes positions, m�langer, et en consommer sans remise :contentReference[oaicite:16]{index=16} */
    ...
    while (1) {
        ...
        if (deck_i > nb_cards) break; /* s�curit� */
        if (deck_i == nb_cards) {
            /* plus de nouvelle carte objectif disponible => partie termin�e :contentReference[oaicite:18]{index=18} */
            break;
        }

        /* tour courant: lire des propositions jusqu�� ce que quelqu�un gagne */
        while (fgets(line, (int)sizeof(line), stdin) != NULL) {
            /* ignorer lignes vides / "." (dans l�annexe on voit un "." tap�) :contentReference[oaicite:19]{index=19} */
            if (line[0] == '\n' || (line[0] == '.' && (line[1] == '\n' || line[1] == '\0'))) {
                continue;
            }
            ...
            if (nread < 2) {
                /* entr�e pas au format "ID SEQ" -> message libre 1 ligne */
            ...
            /* tester la s�quence: on clone depart, on applique, puis on compare � objectif */
            Jeu tmp;
            ...
            if (ok_obj) {
                ...
                /* nouveau d�part = objectif atteint */
                jeu_free(&depart);
                ...
                /* mauvaise s�quence -> joueur �limin� du tour :contentReference[oaicite:20]{index=20} */
                can_play[pj] = 0;
                ...
                if (nb_ok == 1) {
                    ...
                    /* nouveau d�part = objectif (m�me si personne n�a trouv� la s�quence) */
                    jeu_free(&depart);
                    depart = objectif;
